...
#define SYSV
#endif
#if ! ( defined(BSD) || defined(SYSV) || defined(WIN32) || defined(AMIGA) )
...
void pm_init ARGS(( const int * argcP, const char * argv[] ));

/* Variable-sized arrays definitions. */

const char ** pm_allocarray ARGS(( int cols, int rows, int size ));
const char * pm_allocrow ARGS(( int cols, int size ));
void pm_freearray ARGS(( const char ** its, int rows ));
void pm_freerow ARGS(( const char * itrow ));


/* Case-insensitive keyword matcher. */

int pm_keymatch ARGS(( const char * str, const char * keyword, int minchars ));
...
void pm_message ARGS(( const char *, ... ));
void pm_error ARGS(( const char *, ... ));          /* doesn't return */
void pm_perror ARGS(( const char * reason ));           /* doesn't return */
void pm_usage ARGS(( const char * usage ));         /* doesn't return */
...
#ifdef NEED_STRSTR
const char *strstr ARGS((const char *s1, const char *s2));
#endif

#if defined(NEED_VFPRINTF1) || defined(NEED_VFPRINTF2)
int vfprintf ARGS(( FILE* stream, const char * format, va_list args ));
#endif /*NEED_VFPRINTF*/
