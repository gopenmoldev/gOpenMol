...
Leif Laaksonen Center for Scientific Computing 1995, 1996
  ...
  D:\users\leif\Programming\gOpenMol\Probsurf_shared>cl /I..\include /I..\tcl\include 
  probsurf1.2.c /LD  /link /DLL /FORCE:UNRESOLVED /SUBSYSTEM:CONSOLE 
  ..\tcl\lib\tcl81.lib ..\tcl\lib\tk81.lib
...
/* commands */
int lulProbesurf(ClientData , Tcl_Interp *, int , char **);

__declspec(dllexport) int Prob_Init(void);
__declspec(dllexport) int Probsurf_Init(Tcl_Interp *);
...
/*********************************************************************/
int Probsurf_Init(Tcl_Interp *Interp)
/*********************************************************************/
{
int code;
...
/* probesurf program */
Tcl_CreateCommand(Interp,"probesurf",lulProbesurf,(ClientData)NULL,
...
/*********************************************************************/
int lulProbesurf(ClientData clientdata , Tcl_Interp *interp,
int argc , char *argv[])
/*********************************************************************/
{
      float angtau;
...
/* start the show ...*/
      ...
      if(!InputFileSet) {
         ...
         if(NoLogfile) 
            fprintf(Lo_fp,"Input file name not defined"); 
         else
            printf("Input file name not defined"); 
...
/* calculate step-lengths  */
      ...
      if(ProbeSurfMethod) {
        printf("Using squared method (r2)\n");
      } else {
        printf("Using direct method (r)\n");
      }
