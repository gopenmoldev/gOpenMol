#Fchk package for gOpenMol(*).
#Copyright 2003 Kevin J. Boyd, University of New Orleans
#(*)gOpenMol molecular graphics package copyright 1994-2003 Leif Laaksonen
#        CSC - Scientific Computing Ltd, ESPOO, FINLAND
#Permission is granted to distribute and modify this code for personal,
#educational and research use.  

#Provides Tcl wrappers and windows interfaces for fchk.dll procedures.
...
proc drawdipole {dx dy dz} {
	toplevel .dpa
	set FCHKPlugin::dpcolor blue
	label .dpa.dipole -text "X:$dx Y:$dy Z:$dz"
	label .dpa.dscale -text "Length Scale"
	entry .dpa.dscalent -width 10
	label .dpa.radius -text "Arrow Radius"
	entry .dpa.radent -width 10
	button .dpa.setcolor -text "Color" \
	-fg black\
	-bg $FCHKPlugin::dpcolor\
	-state disabled\
	-command {
	}
	button .dpa.draw -text "Draw" -command {
		set dp [getdipole]
		set dx [lindex $dp 0]
		set dy [lindex $dp 1]
		set dz [lindex $dp 2]
		set dsc [.dpa.dscalent get]
		set xx [expr {$dx*$dsc}]
		set yy [expr {$dy*$dsc}]
		set zz [expr {$dz*$dsc}]
		set rad [.dpa.radent get]
		plot arrow 0 0 0 $xx $yy $zz $rad $FCHKPlugin::dpcolor
		destroy .dpa
	}
	grid .dpa.dipole -row 1 -column 2
	grid .dpa.dscale -row 2 -column 1
	grid .dpa.dscalent -row 2 -column 2
	grid .dpa.radius -row 2 -column 3
	grid .dpa.radent -row 2 -column 4
	grid .dpa.draw -row 3 -column 3
	.dpa.dscalent insert 0 1.0
	.dpa.radent insert 0 0.5
}
...
proc fchk {} {
	...
	button .fchkw.filestuff.openfile -text Load -command {
		...
		if {(([string compare $fn ""] != 0) && ($fchkLoaded == 0))} then {
			...
			set dm [getdipole]
			puts $dm
			if {[string compare [lindex $dm 0] "9.9e+099"]} then {
				.fchkw.buttons.dipole configure -state normal
			}
			set fchkLoaded 1
		}
	}
	...
	button .fchkw.buttons.dismiss -text Dismiss -command {set fchkSpin $spin; set fchkOrb $orb; set fchkFilename [.fchkw.filestuff.filename get]; destroy .fchkw}
	button .fchkw.buttons.dipole -text "Draw Dipole Vector" \
		-state disabled -command {
		FCHKPlugin::drawdipole [lindex $dm 0] [lindex $dm 1] [lindex $dm 2]
	}
	...
	pack .fchkw.buttons
	grid .fchkw.buttons.dipole -row 1 -column 1 -in .fchkw.buttons
	grid .fchkw.buttons.dismiss -row 1 -column 2 -in .fchkw.buttons
	set spin $fchkSpin
	set orb $fchkOrb
	if {$spin == 0} then {
